...
if(isset($_POST['submit'])){
    try{
        ...
        $pass = htmlspecialchars(password_hash($_POST['pass'], PASSWORD_BCRYPT, ['cost' => 12]));
        $query = $PDO->prepare("SELECT COUNT(email) AS num FROM users WHERE email = :email");
        $query->bindValue(':email', $email);
        $query->execute();
        $row = $query->fetch(PDO::FETCH_ASSOC);
        if($row['num'] > 0){
            echo '<script>errorModal("error", "Oops...", "Cet email est déjà associé à un compte", "Compris", "<a style=\'opacity:.3\' href=\'login.php\'>J\'ai déjà un compte</a>");</script>';
        }else if($firstName == ''){
            echo '<script>errorModal("error", "Oops...", "Le prénom est obligatoire pour continuer", "Ok", "");</script>';
        }else if(strlen($firstName) < 3){
            echo '<script>errorModal("error", "Oops...", "Le prénom doit contenir 8 caractères", "Ok", "");</script>';
        }else if(strlen($lastName) < 3){
            echo '<script>errorModal("error", "Oops...", "Le nom doit contenir 8 caractères", "Ok", "");</script>';
        }else if($lastName == ''){
            echo '<script>errorModal("error", "Oops...", "Le nom est obligatoire pour continuer", "Ok", "");</script>';
        }else if($email == ''){
            echo '<script>errorModal("error", "Oops...", "L\'email est obligatoire pour continuer", "Ok", "");</script>';
        }else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            echo '<script>errorModal("error", "Oops...", "L\'email n\'est pas correct", "Ok", "");</script>';
        }else if($pass == ''){
            echo '<script>errorModal("error", "Oops...", "Le mot de passe est obligatoire pour continuer", "Ok", "");</script>';
        }else if(strlen($pass) < 8){
            echo '<script>errorModal("error", "Oops...", "Le mot de passe doit contenir 8 caractères", "Ok", "");</script>';
        }else if(!isset($_POST['checkConsent'])){
            echo '<script>errorModal("error", "Oops...", "Les conditions générales de vente ne sont pas accepté", "Ok", "");</script>';
        }else{
...
<section id="registerContainer">
    ...
    <div class="socialContainer">
        ...
        <!-- <div class="buttonFacebookContainer">
            ...
            <input type="button" id="buttonFacebook" onclick='errorModal("error", "Oops...", "Cette option est temporairement indisponible", "Ok", "")'> onclick="window.location='<php echo(isset($facebookLoginUrl) ? $facebookLoginUrl : 'profile'); ?>'"
        </div> -->
    </div>
    ...
    <form method="POST" action="register.php">
        <div class="inputModuleContainer">
            ...
            <input name="lastName" type="text" class="inputModule" value="<?=$_POST['lastName']?>">
        </div>
        <div class="inputModuleContainer">
            <span class="titleInputModule">Prénom</span><br/>
            <input name="firstName" type="text" class="inputModule" value="<?=$_POST['firstName']?>">
        </div>
        <div class="inputModuleContainer">
        ...
        <div class="consentCgvContainer">
            ...
            <label for="checkConsent">
                Je reconnais avoir pris connaissance et j’accepte les <a class="redBoldText" href="legal" target="_blank">conditions générales de vente</a>
            </label>
        </div>
        <input name="submit" type="submit" value="C'est parti !" id="registerSubmitButton">
    </form>
    <hr style="margin: 24px 0 0 0;">
    <div class="bottomPart">
        Vous avez déjà un compte ?<br/>
        <a href="login.php" class="redBoldText" id="loginButton">Se connecter</a>
    </div>
</section>
